...
// v21: Service Worker registration disabled while the site is changing frequently.
...
// v21: actively unregister old Service Workers on pages that still have one controlling them.
(function(){
  ...
  window.addEventListener('load', function(){
    ...
    if(window.caches){ caches.keys().then(function(keys){ keys.forEach(function(key){ caches.delete(key); }); }).catch(function(){}); }
  });
})();

// v23: TikTok Lite cluster uses existing copy, analytics, and SW cleanup handlers.

// v26 minimal premium conversion: stable JS, no class rewrites.

// v27: minimal clear UI. No sticky navigation.
